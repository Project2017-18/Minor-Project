...
##
            ...
            #X_train, X_test, y_train, y_test = train_test_split(Xtemp,Y1,train_size=0.5,random_state=25)
            fpr, tpr, thresholds = roc_curve(y_test, y_score[:,0],pos_label=0)
            roc_auc = auc(fpr, tpr)
            lw = 2
...
##                B=model.fit(Xtemp[train], Y[train])
##                prediction=B.predict(Xtemp[test].reshape((n_samples, n_features )))
##                acc = accuracy_score(Y[test], prediction)
##                accuracy=accuracy+acc
##                #m=matthews_corrcoef(Y[test], prediction)
##                #mcc=mcc+m
...
##            y_score=model.predict_proba(X_test)
##            #X_train, X_test, y_train, y_test = train_test_split(Xtemp,Y1,train_size=0.5,random_state=25)
##            fpr, tpr, thresholds = roc_curve(y_test, y_score[:,0])
##            roc_auc = auc(fpr, tpr)
##            lw = 2
##            plt.plot(fpr, tpr, color='darkorange',
##             lw=lw, label='ROC curve (area = %0.2f)' % roc_auc)
