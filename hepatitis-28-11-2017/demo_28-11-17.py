...
##    X=X[:,1:]
    ...
    recall_box_plot=[]
    f1score_box_plot=[]
    names=['Random Forest','Desicion Tree','Boosting','Bagging']
    for model, label,sheet in zip([ model2,model5,model6,model7],names,['sheet1','sheet2','sheet3','sheet4']):
...
##        loo = LeaveOneOut()
        ...
        fpr, tpr, thresholds = roc_curve(y_test, y_score[:,0], pos_label=1)
        roc_auc = auc(fpr, tpr)
        lw = 2
