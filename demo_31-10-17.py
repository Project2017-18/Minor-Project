...
for data ,outfile in zip(datasets,xlsxfile):
    ...
    for model, label,sheet in zip([ model1,model2],names,['sheet1','sheet2']):
        ...
        for i in features_list:
            ...
            accuracy_list.append(accuracy)
            #matthews_corrcoef_list.append(mcc)
            CMIM_ERROR_LIST.append(1-accuracy)
            precision_list.append( precision)
            recall_list.append(recall)
            f1_list.append(f1score)
            time_list.append(float(time.time()-start))
...
##            
            ...
            for train, test in ss:
                ...
                X_train=X[train]
                X_test=X[test]
                score1=chi_square.chi_square(X[train],Y[train])
        ...
        for elist ,name in zip([CMIM_ERROR_LIST,MRMR_ERROR_LIST,ICAP_ERROR_LIST,DISR_ERROR_LIST,MIM_ERROR_LIST,MIFS_ERROR_LIST,JMI_ERROR_LIST,CIFE_ERROR_LIST,LCSI_ERROR_LIST,
                                fisher_score_ERROR_LIST,reliefF_ERROR_LIST,SPEC_ERROR_LIST,TRACE_RATIO_ERROR_LIST,LAP_SCORE_ERROR_LIST,
                                CHI_SQUARE_ERROR_LIST,F_SCORE_ERROR_LIST,GINI_INDEX_ERROR_LIST,LOW_VARIANCE_ERROR_LIST],feature_list[1:]):
            ...
            plt.legend(loc="lower right")
            plt.grid()
            plt.savefig('error_bar_'+label+'_'+name+'_'+str(data)[:-4])
            plt.gcf().clear()
